<?php
/*
Single Post Template: Landwash Post
*/
?>
...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
		<div id="indy_feature_preview_singlepost">
			<h1 style="padding: 0px 0px 0px 20px;"><a style="color: orange; text-shadow: -1px 1px #404040;" href="<?php the_permalink(); ?>"><?php the_title() ?></a></h1>
				<div id="indy_sublines_singlepost" style="background: none; color: #c0c0c0; width:352px; padding: 4px 4px 4px 19px;">
				By: <?php if (get_post_meta($post->ID, 'Guest Author Name', true) != "") {echo get_post_meta($post->ID, 'Guest Author Name', true);} else {the_author_posts_link(); } ?> | <?php the_time('F j, Y'); ?>
					<div id="indy_sharing"><div style="margin: 5px 0px 2px 0px;"><div style="float: left; margin: 0px; padding: 0px;"><?php dd_twitter_generate('Compact','twitter_username') ?></div><div style="float: left; margin: 0px; padding: 0px; width: 70px; overflow: hidden;"><?php dd_google1_generate('Compact (20px)') ?></div><div style="float: left; margin: 0px; padding: 0px;"><?php dd_fblike_generate('Recommend Button Count') ?></div></div></div>
		</div>		

				<div id="indy_text_singlepost" style="color: white; text-shadow: -1px 1px #404040;">
					<p><?php the_excerpt(); ?></p>
